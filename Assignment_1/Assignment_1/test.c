...
double* Populate_mat1(double* mat1, FILE *matrix_1,int rows,int columns )
{
	...
	while((fgets(string_buffer, MAX_LINE_LENGTH, matrix_1)) !=NULL)
	{
		...
		else if(strstr(string_buffer, "matrix"))
		{
			...
			for (i=0; i<rows+1; i++) {
				if (string_buffer[rows+1] != NULL) {
					printf("Number of rows in input does not match the specified value");
					exit(BAD_FORMAT);
				}
				for (j=0; j<columns+1; j++) {
					if (string_buffer[columns+1] != NULL) {
						printf("Number of columns in input does not match the specified value");
						exit(BAD_FORMAT);
					}
